...
# ---------------------------
# Training routine (with immediate stop & GPU optimizations)
# ---------------------------
def train_worker(data_dir, arch_name, epochs, batch_size, lr, save_path, log_callback, stop_event, progress_callback=None):
    try:
        ...
        if cuda_avail:
            try:
                # print cuda details for debugging
                dev_idx = torch.cuda.current_device()
                dev_name = torch.cuda.get_device_name(dev_idx)
                torch.backends.cudnn.benchmark = True
                log_callback(f"CUDA device available: {dev_name} (index {dev_idx})")
            except Exception as e:
                log_callback(f"CUDA detection: {e}")
        ...
        for epoch in range(epochs):
            if stop_event.is_set():
                log_callback("Training stopped before epoch start.")
                if progress_callback:
                    progress_callback(0, 1, 0.0, epoch+1, epochs)
                return
            ...
            if total_batches == 0:
                log_callback("No batches found in train_loader.")
                return

            log_callback(f"Starting epoch {epoch+1}/{epochs} (batches: {total_batches})")
            for batch_idx, (images, labels) in enumerate(train_loader):
                # immediate stop check inside batch loop
                if stop_event.is_set():
                    log_callback("Stop signal detected inside epoch; aborting training now.")
                    if progress_callback:
                        progress_callback(batch_idx, total_batches, running_loss / max(1, batch_idx), epoch+1, epochs)
                    return

                # move data to device with non_blocking if using pinned memory
                non_block = True if torch.cuda.is_available() else False
                images = images.to(device, non_blocking=non_block)
                ...
                avg_loss_so_far = running_loss / current_batch

                # update progress UI
                if progress_callback:
                    progress_callback(current_batch, total_batches, avg_loss_so_far, epoch+1, epochs)
            ...
            with torch.no_grad():
                for images, labels in val_loader:
                    # if user requested stop during validation; abort
                    if stop_event.is_set():
                        log_callback("Stop signal detected during validation; aborting.")
                        if progress_callback:
                            progress_callback(0, total_batches, 0.0, epoch+1, epochs)
                        return
        ...
        if progress_callback:
            progress_callback(0, 1, 0.0, epochs, epochs)
    except Exception as e:
...
# ---------------------------
def extract_faces_with_mediapipe(video_path, output_folder, face_size=(224,224), frame_skip=1, padding=0.5, log_callback=None, stop_event=None):
    ...
    with mp_face_detection.FaceDetection(model_selection=1, min_detection_confidence=0.4) as face_detection:
        while cap.isOpened():
            if stop_event is not None and stop_event.is_set():
                if log_callback:
                    log_callback("Stop signal received during face extraction.")
                break
...
class DeepfakeApp(tk.Tk):
    ...
    # -----------------
    # Train tab UI (with progress bar)
    # -----------------
    def build_train_tab(self, parent):
        ...
        ttk.Label(top, text="Save model location (choose folder):").grid(row=5, column=0, sticky="w", **pad)
        ...
        # progress bar & label under buttons
        progress_frame = ttk.Frame(parent)
        progress_frame.pack(fill="x", padx=8, pady=(4,8))
        self.train_progress = ttk.Progressbar(progress_frame, orient="horizontal", length=600, mode="determinate")
        self.train_progress.pack(side="left", padx=(0,8))
        self.train_progress_label = ttk.Label(progress_frame, text="Idle")
        self.train_progress_label.pack(side="left")
    ...
    # Modified: choose folder then auto create filename
    ...
    def start_training_thread(self):
        ...
        try:
            ...
            messagebox.showerror("Input error", "Please enter valid numeric values for epochs, batch size and learning rate.")
            return
        ...
        if not os.path.isdir(data_dir):
            ...
            return
        if not save_path:
            messagebox.showerror("Save path", "Please choose a location (folder) to save the trained model.")
            return

        self.stop_event.clear()

        def progress_cb(current_batch, total_batches, avg_loss, epoch_number, total_epochs):
            def _update():
                if total_batches <= 0:
                    self.train_progress['value'] = 0
                    self.train_progress['maximum'] = 1
                else:
                    self.train_progress['maximum'] = total_batches
                    self.train_progress['value'] = current_batch
                if current_batch == 0:
                    self.train_progress_label.config(text=f"Epoch {epoch_number}/{total_epochs} - Idle")
                else:
                    self.train_progress_label.config(text=f"Epoch {epoch_number}/{total_epochs} - batch {current_batch}/{total_batches}  avg_loss: {avg_loss:.4f}")
            self.after(0, _update)

        t = threading.Thread(target=train_worker,
                             args=(data_dir, arch, epochs, batch_size, lr, save_path, self.log, self.stop_event, progress_cb),
                             daemon=True)
        t.start()
        self.log(f"Training started (arch={arch}, epochs={epochs}, batch={batch_size}, lr={lr}).")

    def stop_training(self):
        # set the stop flag so worker stops ASAP (inside batch loop)
        self.stop_event.set()
        self.log("Stop signal sent to training.")
        # UI feedback
        def reset():
            self.train_progress_label.config(text="Stopping...")
        self.after(50, reset)
    ...
    def video_prediction_worker(self, video_path, model_path, fallback_arch, frame_skip, padding):
        ...
        count = extract_faces_with_mediapipe(video_path, tmpdir, face_size=(224,224), frame_skip=frame_skip, padding=padding, log_callback=self.log, stop_event=self.stop_event)
        if count == 0:
        ...
        for fn in os.listdir(tmpdir):
            if self.stop_event.is_set():
                self.log("Stop requested; aborting face predictions.")
                break
